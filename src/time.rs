...
impl ToTime for Decimal {
    fn to_time(&self) -> Time {
        ...
        let mut time_builder = TimeBuilder::new();
...
mod tests {
    ...
    fn time(hours: u64, minutes: u8, seconds: u8, nanoseconds: u32) -> Time {
        TimeBuilder::new().hours(hours).minutes(minutes).seconds(seconds).nanoseconds(nanoseconds).build()
    }

    fn neg_time(hours: u64, minutes: u8, seconds: u8, nanoseconds: u32) -> Time {
        TimeBuilder::new().negative().hours(hours).minutes(minutes).seconds(seconds).nanoseconds(nanoseconds).build()
    }
